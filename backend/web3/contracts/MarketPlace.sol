...
contract Marketplace {
    ...
    constructor(uint256 _projectCreationFee) {
        require(_projectCreationFee >= 0, "Project creation fee must be positive");
        memo = payable(msg.sender);
        projectCreationFee = _projectCreationFee;
    }

    //Structures
    struct Project { //Project Structure
    ...
    //Events
    event ProjectCreated( //Event for when a project is created
    ...
    function checkSharesArray(uint[] memory _shares) private returns(bool){
        uint sum;
    ...
    function createProject(
        ...
        require(checkSharesArray(shares), "Sum of royalty distribution must be equal to 100");
    ...
    function updateProjectCreationFee(uint _newFee) public onlyMemo {
